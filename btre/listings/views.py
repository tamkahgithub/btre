...
def listing(request, listing_id):
    ...
    listings = get_objects_or_404(Listing, pk=listing_id)
    context = {
        'listing': listing
    }
...
def search(request):
    ...
    if 'keywoeds' in request.GET:
        keywords = request.GET['keywords']
        if keywords:
            queryset_list = queryset_list.filter(desiption__icontains=keywords)
    ...
    # search price with less than option
